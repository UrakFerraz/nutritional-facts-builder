import { IngredientInterface } from './Ingredient'

export interface NutritionalValues {
  calories: IngredientInterface
  carbohydrate: IngredientInterface
  protein: IngredientInterface
  totalFat: IngredientInterface
  saturatedFat: IngredientInterface
  transFat: IngredientInterface
  cholesterol: IngredientInterface
  dietaryFibers: IngredientInterface
  sodium: IngredientInterface
  calcium: IngredientInterface
  biotin: IngredientInterface
  choline: IngredientInterface
  chrome: IngredientInterface
  copper: IngredientInterface
  fluorine: IngredientInterface
  folicAcid: IngredientInterface
  iodine: IngredientInterface
  iron: IngredientInterface
  magnesium: IngredientInterface
  manganese: IngredientInterface
  molybdenum: IngredientInterface
  niacin: IngredientInterface
  pantothenicAcid: IngredientInterface
  phosphorus: IngredientInterface
  potassium: IngredientInterface
  riboflavin: IngredientInterface
  saturatedFattyAcids: IngredientInterface
  selenium: IngredientInterface
  thiamine: IngredientInterface
  vitaminA: IngredientInterface
  vitaminB12: IngredientInterface
  vitaminB6: IngredientInterface
  vitaminC: IngredientInterface
  vitaminD: IngredientInterface
  vitaminE: IngredientInterface
  vitaminK: IngredientInterface
  zinc: IngredientInterface
}
